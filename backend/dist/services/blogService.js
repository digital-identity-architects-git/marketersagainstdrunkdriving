...
export async function generateDailyBlog(incidents) {
    const topIncidents = incidents.slice(0, 10);
    const incidentSummary = topIncidents
        .map((inc, idx) => `${idx + 1}. ${inc.title} in ${inc.location}, ${inc.state} - ${inc.deathCount} deaths, ${inc.injuryCount} injuries`)
        .join('\n');
    const culturalRefs = [
...
Format as JSON array with fields: platform, text`;
    ...
    try {
        ...
        socialPosts = Array.isArray(parsed) ? parsed.map((p) => p.text || '') : [];
    }
    catch {
...
export async function generateIncidentTitle(incident) {
